...
from jafs.tools.base.tool import BaseTool
...
class CalculatorTool(BaseTool):
    ...
    JAFS can handle your numbers with precision and care.
    """
    ...
    def execute(self, expression: str = None, **kwargs) -> Union[Dict[str, Any], ToolResult]:
        """
        ...
        """
        try:
            # Handle the case where expression might be passed differently
            if expression is None:
                # Check if expression is in kwargs
            ...
            # Add some JAFS flair for certain numbers
            if "42" in clean_expr:
                logging.info("JAFS calculator triggered an easter egg: 42")
            elif "69" in clean_expr:
                logging.info("JAFS calculator is keeping it professional...")
            ...
            # Add some JAFS humor based on the result
            if result == 69:
                logging.info("JAFS calculator is maintaining its composure...")
            elif result == 404:
                logging.info("JAFS calculator lost something in the backend...")
            elif result == 42:
                logging.info("JAFS calculator found the meaning of life!")
    ...
    def _eval_expr(self, node: ast.AST) -> float:
        ...
        """
        # Numbers - handle both old-style Num and new-style Constant nodes
        ...
        # Binary operations (e.g., 2 + 3, 4 * 5)
            ...
            if isinstance(node.op, ast.Div) and right == 0:
                raise ValueError("JAFS cannot divide by zero!")
